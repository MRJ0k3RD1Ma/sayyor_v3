...
namespace app\models\search\laboratory;
...
class FoodRouteSearch extends FoodRoute
{
    ...
    {
        $query = FoodRoute::find()->where(['executor_id' => Yii::$app->user->id])->orderBy(['created' => SORT_DESC]);
        ...
        if ($this->q) {
            $query->joinWith('sample')
                ->andFilterWhere([
                    'like', 'food_samples.samp_code', $this->q
                ]);
        }
