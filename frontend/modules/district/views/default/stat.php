    ...
    'columns' => [
        ...
        [
            ...
            'value' => function ($SoatoModel) {
                return Html::a($SoatoModel->name_lot, ['/district/stat', 'id' => $SoatoModel->district_id]);
            }
        ],
        [
            ...
            'value' => function ($SoatoModel) use ($AnimalDataProvider) {
                ...
                foreach ($AnimalDataProvider->getModels() as $model) {
                    $counter[$model->status->id] += Yii::$app->request->get('id') ? ($model->vetSite->soato0->district_id == $SoatoModel->district_id) : ($model->vetSite->soato0->district_id == $SoatoModel->district_id);
                }
        ...
        [
            ...
            'value' => function ($SoatoModel) use ($FoodDataProvider) {
                ...
                foreach ($FoodDataProvider->getModels() as $model) {
                    ...
                    $counter[$model->status_id] += Yii::$app->request->get('id') ? ($model->soato0->district_id == $SoatoModel->district_id) : ($model->soato0->district_id == $SoatoModel->district_id);
                }
                foreach (SertStatus::find()->all() as $status) {
                    $out[] = Html::tag('i', $status->name_uz . ": " . $counter[$status->id]);
                }
        ...
        [
            ...
            'value' => function ($SoatoModel) use ($AnimalRegProvider) {
                ...
                foreach ($AnimalRegProvider->getModels() as $model) {
                    if ($model->inn) {
                        $counter[$model->status_id] += Yii::$app->request->get('id') ? ($model->inn0->soato->district_id == $SoatoModel->district_id) : ($model->inn0->soato->district_id == $SoatoModel->district_id);
                    } elseif ($model->pnfl) {
                        $counter[$model->status_id] += Yii::$app->request->get('id') ? ($model->pnfl0->soato->district_id == $SoatoModel->district_id) : ($model->pnfl0->soato->district_id == $SoatoModel->district_id);
                    }
                }
                foreach (SertStatus::find()->all() as $status) {
                    $out[] = Html::tag('i', $status->name_uz . ": " . $counter[$status->id]);
